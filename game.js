...
/* ---------- Haptik (Vibration) ----------
   Hinweis: Safari/iOS unterstützt die Web-Vibration-API bis heute nicht
   (Apple hat sie nie implementiert, auch nicht in installierten PWAs) —
   auf dem iPhone bleibt das also stumm. Der Code ist trotzdem sinnvoll:
   er schadet nirgends (reines Feature-Detection, keine Fehler), und
   falls die App jemals auf Android/Chrome läuft, funktioniert die
   Vibration dort sofort. */
const Haptics = {
  supported: typeof navigator !== 'undefined' && typeof navigator.vibrate === 'function',
  buzz(pattern) {
    if (!saveData.soundOn) return; // an dieselbe Einstellung wie Sound gekoppelt
    if (this.supported) { try { navigator.vibrate(pattern); } catch (e) {} }
  },
  match(comboLevel) { this.buzz(comboLevel >= 3 ? [12, 20, 12] : 10); },
  special() { this.buzz([14, 30, 14]); },
  win() { this.buzz([20, 40, 20, 40, 30]); },
  lose() { this.buzz(60); }
};
...
const WORLDS = [
  ...
  { name: 'Nebelwald', bg1: '#1a2e1f', bg2: '#3a5a42', blob1: '#8fffb0', blob2: '#c9ff5f', hue: 100 },
  { name: 'Kupferwüste', bg1: '#3a2410', bg2: '#7a4a1f', blob1: '#ffb347', blob2: '#ff7a5c', hue: -70 },
  { name: 'Eisspalte', bg1: '#16324a', bg2: '#3d6f92', blob1: '#c9f3ff', blob2: '#7fd6ff', hue: 190 },
  { name: 'Schattenreich', bg1: '#0a0612', bg2: '#1f1030', blob1: '#7a2fff', blob2: '#ff2fae', hue: 300 },
  { name: 'Korallenriff', bg1: '#1a2f3a', bg2: '#2f6a6a', blob1: '#ff9ac9', blob2: '#5fe8d0', hue: 20 }
];
...
/* Punktesystem: der Zielwert wächst mit der Wurzel aus der Levelnummer,
   NICHT asymptotisch gegen eine feste Obergrenze (das war der Kern des
   Problems — vorher blieb das Ziel ab ca. Level 60-80 für immer gleich
   hoch, während Spieler mit der Zeit routinierter werden und größere
   Kombos schaffen, wodurch spätere Level trivial leicht wurden). Die
   Wurzel-Kurve steigt dagegen ohne Obergrenze weiter, aber mit
   abnehmender Rate pro Level — ein spürbarer, aber fairer Dauerantrieb
   über beliebig viele Level hinweg. */
const DIFFICULTY_PRESETS = {
  leicht: { baseTarget: 340, growthRate: 24, baseMoves: 28, minMoves: 18, moveDecayK: 6 },
  mittel: { baseTarget: 430, growthRate: 29, baseMoves: 25, minMoves: 15, moveDecayK: 6 },
  schwer: { baseTarget: 540, growthRate: 34, baseMoves: 22, minMoves: 13, moveDecayK: 6 }
};
function buildLevel(n, difficulty) {
  const preset = DIFFICULTY_PRESETS[difficulty] || DIFFICULTY_PRESETS.mittel;
  let target = Math.round(preset.baseTarget + preset.growthRate * Math.sqrt(n - 1));
...
class Game {
  ...
  _createEl(cell, r, c) {
    ...
    if (cell.key) {
      ...
      shaft.className = 'key-shaft';
      const tooth = document.createElement('span');
      tooth.className = 'key-tooth';
      key.appendChild(ring);
      key.appendChild(shaft);
      key.appendChild(tooth);
      el.appendChild(key);
    }
  ...
  /* Kleines Feuerwerk beim Auflösen eines Kristalls: kurzer Lichtblitz im
     Zentrum plus ein Kranz bunter, unterschiedlich großer Funken. */
  _burstParticles(r, c, type) {
    ...
    const altColor = GEM_COLORS[GEM_TYPES[(GEM_TYPES.indexOf(type) + 3) % GEM_TYPES.length]] || color;
    const sparkColors = [color, '#ffffff', altColor];
    const n = 10;
    for (let i = 0; i < n; i++) {
      ...
      const dist = this.cellSize * (0.7 + Math.random() * 0.7);
  ...
  _clearCells(cellsArr, multiplier) {
    ...
    if (count > 0) {
      const gained = Math.round(count * 8 * multiplier);
      this.score += gained;
  ...
  async _resolveCascades(preferredPos) {
    ...
    while (true) {
      ...
      // Feintuning der Punkte-Balance: der Kombo-Multiplikator wächst jetzt
      // gedämpft (statt 1:1 mit der Kettenlänge), damit auch sehr lange
      // Kettenreaktionen nicht unverhältnismäßig viele Punkte auf einmal
      // ausschütten — lange Ketten bleiben klar lohnender, aber ohne den
      // Punktestand schlagartig explodieren zu lassen.
      const scoreMult = this.combo <= 1 ? 1 : 1 + (this.combo - 1) * 0.55;
      this._clearCells(cellsToClear, scoreMult);
...
function renderLevelMap(scrollToCurrent) {
  ...
  for (let i = levelMapRangeStart; i <= saveData.unlocked; i++) {
    ...
    const preview = buildLevel(i, saveData.difficulty);
    if (preview.isBoss) {
      const badge = document.createElement('span');
      badge.className = 'lv-badge lv-badge-boss';
      node.appendChild(badge);
    } else if (preview.objective !== 'score') {
      const badge = document.createElement('span');
      badge.className = 'lv-badge lv-badge-' + preview.objective;
      node.appendChild(badge);
    }
