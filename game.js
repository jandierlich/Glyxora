...
const SoundManager = {
  ...
  tone(freq, dur, type = 'sine', gainVal = 0.08, delay = 0) {
  ...
  match(comboLevel) {
    ...
    this.tone(base, 0.16, 'sine', 0.07);
  },
  special() {
    this.tone(300, 0.1, 'square', 0.05);
    this.tone(600, 0.18, 'sine', 0.06, 0.05);
  },
  win() {
    [523, 659, 784, 1046].forEach((f, i) => this.tone(f, 0.22, 'sine', 0.07, i * 0.1));
  },
  lose() {
...
function defaultSaveData() {
  return {
    unlocked: 1, stars: {}, zenBest: 0, soundOn: true, difficulty: 'mittel',
    jokers: { undo: 3, auto: 3, color: 2, extra: 2 },
    stats: {
      levelsCompleted: 0, totalScore: 0, bestCombo: 1,
...
const WORLDS = [
  { name: 'Kristallhöhle', bg1: '#1a1436', bg2: '#2c1f57', blob1: '#ff5da2', blob2: '#39d6ff' },
  { name: 'Wolkenreich', bg1: '#2c4470', bg2: '#5f8fc4', blob1: '#ffffff', blob2: '#ffe08a' },
  { name: 'Vulkanfeld', bg1: '#3a1210', bg2: '#7a2b12', blob1: '#ff8a3c', blob2: '#ffdf3c' },
  { name: 'Tiefsee', bg1: '#0a3244', bg2: '#146a7a', blob1: '#4fd6c8', blob2: '#2f95ff' },
  { name: 'Sternenhimmel', bg1: '#0a0a2a', bg2: '#241a4e', blob1: '#ffd23f', blob2: '#b25bff' }
];
...
function buildLevel(n, difficulty) {
  ...
  if (!isBoss && n >= 10 && n % 5 === 0) {
    objective = (Math.floor(n / 5) % 2 === 0) ? 'keys' : 'ice';
  }
...
class Game {
  ...
  _makeGem(type) {
    return { id: this.nextId++, type, special: null, iceLevel: 0, key: false };
  }
  ...
  _createEl(cell, r, c) {
    ...
    face.style.setProperty('--delay', delay);
    face.innerHTML = '<span class="eye eye-l"></span><span class="eye eye-r"></span><span class="mouth"></span>';
  ...
  _clearCells(cellsArr, multiplier) {
    ...
    for (const [r, c] of cellsArr) {
      ...
      this._burstParticles(r, c, cell.type);
...
function onLevelEnd(cfg, won) {
  ...
  setTimeout(() => Overlays.show('overlay-result'), 400);
}
...
function bindUI() {
  ...
  document.getElementById('btn-reset-progress').addEventListener('click', () => {
    if (confirm('Wirklich den gesamten Fortschritt zurücksetzen?')) {
      saveData = { unlocked: 1, stars: {}, zenBest: 0, soundOn: SoundManager.enabled };
      Storage.save(saveData);
      updateHomeStats();
